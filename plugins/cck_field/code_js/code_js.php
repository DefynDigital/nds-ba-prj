	...
	public function onCCK_FieldPrepareContent( &$field, $value = '', &$config = array() )
	{
		...
		if ( $field->state && $field->defaultvalue != '' ) {
			...
			if ( strpos( $field->defaultvalue, '$cck->get' ) !== false ) {
				...
				if ( count( $matches[1] ) ) {
					parent::g_addProcess( 'beforeRenderContent', self::$type, $config, array( 'name'=>$field->name, 'matches'=>$matches, 'js'=>$field->defaultvalue ) );
				}
			} else {
	...
	public function onCCK_FieldPrepareForm( &$field, $value = '', &$config = array(), $inherit = array(), $return = false )
	{
		...
		if ( $field->state && $field->defaultvalue != '' ) {
			...
			if ( strpos( $field->defaultvalue, '$cck->get' ) !== false ) {
				...
				if ( count( $matches[1] ) ) {
					parent::g_addProcess( 'beforeRenderForm', self::$type, $config, array( 'name'=>$field->name, 'matches'=>$matches, 'js'=>$field->defaultvalue ) );
				}
			} else {
				if ( JFactory::getApplication()->input->get( 'tmpl' ) == 'raw' ) {
					echo '<script type="text/javascript">'.$js.'</script>';
