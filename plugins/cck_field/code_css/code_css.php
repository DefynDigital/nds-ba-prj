	...
	public function onCCK_FieldPrepareContent( &$field, $value = '', &$config = array() )
	{
		...
		if ( $field->state && $field->defaultvalue != '' ) {
			if ( strpos( $field->defaultvalue, '$cck->get' ) !== false ) {
				...
				if ( count( $matches[1] ) ) {
					parent::g_addProcess( 'beforeRenderContent', self::$type, $config, array( 'name'=>$field->name, 'matches'=>$matches, 'css'=>$field->defaultvalue ) );
				}
			} else {
				if ( JFactory::getApplication()->input->get( 'tmpl' ) == 'raw' ) {
					echo '<style type="text/css">'.$field->defaultvalue.'</style>';
				} else {
					JFactory::getDocument()->addStyleDeclaration( $field->defaultvalue );
				}
			}
		}
	}
	...
	public function onCCK_FieldPrepareForm( &$field, $value = '', &$config = array(), $inherit = array(), $return = false )
	{
		...
		if ( $field->state && $field->defaultvalue != '' ) {
			if ( strpos( $field->defaultvalue, '$cck->get' ) !== false ) {
				...
				if ( count( $matches[1] ) ) {
					parent::g_addProcess( 'beforeRenderForm', self::$type, $config, array( 'name'=>$field->name, 'matches'=>$matches, 'css'=>$field->defaultvalue ) );
				}
			} else {
				if ( JFactory::getApplication()->input->get( 'tmpl' ) == 'raw' ) {
					echo '<style type="text/css">'.$field->defaultvalue.'</style>';
